...
with driver.session() as session:
    ...
    for record in result:
        city_data[record["id"]] = {"lat": record["lat"], "lon": record["lon"], "name": record["name"] or "?"}
...
m.scatter(x_cities, y_cities, s=30, color="royalblue", edgecolor="navy", zorder=10, label=f"{len(city_data):,} Cities")
...
for start_id, end_id in roads:
    ...
    x, y = m(lons, lats)
    m.plot(x, y, color="tomato", linewidth=1.1, alpha=0.7, zorder=5)  # roads under cities
