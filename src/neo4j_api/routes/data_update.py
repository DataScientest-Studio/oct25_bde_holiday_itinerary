...
from src.neo4j_api.data_upload_etl import unzip_data, perform_extract_data
from src.neo4j_api.datatourisme_handler import check_download, perform_download, NoDataAvailable, AuthenticatedClient
...
async def extract_data(bg_tasks: BackgroundTasks) -> Dict[str, Any]:
    try:
        ...
        raise HTTPException(
            status_code=409,
            detail="Extract already in progress"
        )
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc))
